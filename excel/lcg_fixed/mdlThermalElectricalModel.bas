...
'===============================================================================
' Global Portable Random Number Generator
'===============================================================================
Public g_PortableRNG As clsPortableLCG
...
Public Sub InitializePortableRNG(Optional Seed As Long = 1)
    ...
    '
    Set g_PortableRNG = New clsPortableLCG
    g_PortableRNG.Initialize Seed
End Sub

 Public Sub InitDebugLog()
      On Error Resume Next
      If g_LogFile > 0 Then Close #g_LogFile  ' Close if already open
      On Error GoTo 0

      g_LogCount = 0
      g_LogFile = FreeFile  ' Get next available file number
      Open "C:\Users\James\Desktop\random_debug.txt" For Output As #g_LogFile

      If Err.Number <> 0 Then
          MsgBox "Error opening log file: " & Err.Description
      End If
  End Sub
...
Sub RunThermalElectricalDemandModel()
    ...
    ' Initialize portable RNG FIRST - before anything else!
...
Private Sub AssignDwellingParameters(dwellingIndex As Integer)
    ...
    DebugLog_Random ("mdlThermalElectricalModel:1195 - Select number of residents")
    dblRand = g_PortableRNG.Random()
    dblCumulativeP = 0
    ...
    DebugLog_Random ("mdlThermalElectricalModel:1208 - Select building index")
    dblRand = g_PortableRNG.Random()
    dblCumulativeP = 0
    With wsBuildings
    ...
    DebugLog_Random ("mdlThermalElectricalModel:1221 - Select heating system index")
    dblRand = g_PortableRNG.Random()
    dblCumulativeP = 0
    With wsPrimaryHeatingSystems
    ...
    DebugLog_Random ("mdlThermalElectricalModel:1221 - Select pv system index")
    dblRand = g_PortableRNG.Random()
    dblCumulativeP = 0
    With wsPVSystems
    ...
    Else
        DebugLog_Random ("mdlThermalElectricalModel - Select solar thermal index")
        dblRand = g_PortableRNG.Random()
        dblCumulativeP = 0
        With wsSolarThermal
    ...
    DebugLog_Random ("mdlThermalElectricalModel:1251 - Select cooling system index")
    dblRand = g_PortableRNG.Random()
    dblCumulativeP = 0
    With wsCoolingSystems
...
'===============================================================================
' Portable Normal Distribution Functions
'
' Replace ALL normal distribution sampling with these standardized functions.
' Uses inverse CDF method (NormInv) for exactly 1 random call per value.
' This ensures synchronization with Python's scipy.stats.norm.ppf.
'===============================================================================

' ===============================================================================
' DEPRECATED: GetMonteCarloNormalDistGuess
' Replaced with GetPortableNormalInteger for cross-platform validation
' Old rejection sampling method used variable number of random calls (2-10+)
' New inverse CDF method uses exactly 1 random call per value
' ===============================================================================

' ===============================================================================================
' ## GetPortableNormalValue (REPLACEMENT FOR GetMonteCarloNormalDistGuess)
' Generate a value from a normal distribution using inverse CDF method
' Uses exactly 1 random call for deterministic cross-platform validation
' ===============================================================================================
Public Function GetPortableNormalValue(dMean As Double, dSD As Double) As Double

      ' Special case: If SD is 0 or very small, return the mean
      ' IMPORTANT: Also uses ZERO random calls
      If dSD <= 0.0000001 Then
          GetPortableNormalValue = dMean
          DebugLog_Random "rN2"
          Exit Function
      End If
      ...
      ' Get uniform random value (exactly 1 call)
      u = g_PortableRNG.Random()

      ' Clamp to avoid NormInv errors
      If u <= EPSILON Then u = EPSILON
      If u >= (1# - EPSILON) Then u = 1# - EPSILON

      GetPortableNormalValue = Application.WorksheetFunction.NormInv(u, dMean, dSD)
  End Function
...
' ===============================================================================================
' ## GetPortableNormalInteger (REPLACEMENT FOR GetMonteCarloNormalDistGuess)
' Generate an INTEGER value from a normal distribution
' ===============================================================================================
  Public Function GetPortableNormalInteger(dMean As Double, dSD As Double) As Integer
      ' Special case: If mean is 0, return 0
      If dMean = 0 Then
          GetPortableNormalInteger = 0
          DebugLog_Random "rN3"
          Exit Function
      End If
      ...
      ' Special case: If SD is 0 or very small, return the mean
      If dSD <= 0.0000001 Then
          GetPortableNormalInteger = CInt(dMean)
          DebugLog_Random "rN4"
          Exit Function
      End If
