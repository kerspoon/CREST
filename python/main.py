...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory to save simulation results (default: output/run_YYYYMMDD_NN/)"
    )
    parser.add_argument(
        "--save-detailed",
        action="store_true",
    ...
    if args.output_dir is None:
        # Import utilities for auto-numbered output directory
        scripts_dir = Path(__file__).parent.parent / "scripts"
        sys.path.insert(0, str(scripts_dir))
        try:
            from utils import create_output_dir
            args.output_dir = create_output_dir("", prefix="run")
    ...
    if args.config_file:
        ...
        for dwelling_idx, dwelling_config in enumerate(dwelling_configs):
            ...
            results.append({
                ...
                'lighting_kwh': lighting_demand_kwh,
                'appliances_kwh': appliance_demand_kwh,
                'pv_output_kwh': pv_output_kwh,
                'total_electricity_kwh': total_electricity_kwh,
                'self_consumption_kwh': self_consumption_kwh,
                'net_electricity_kwh': net_electricity_kwh,
                'hot_water_litres': hot_water_litres,
                'average_indoor_temp': average_indoor_temp,
                'thermal_energy_space_kwh': thermal_energy_space_kwh,
                'thermal_energy_water_kwh': thermal_energy_water_kwh,
                'gas_m3': gas_m3,
                'space_setpoint': space_setpoint,
        ...
        # VBA: For each dwelling, generate config THEN simulate immediately
